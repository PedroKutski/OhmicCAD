...
const DEFAULT_LED_IS = 5e-12;
const DEFAULT_LED_IDEALITY = 1.9;
const DEFAULT_LED_INTERNAL_RS = 1.5;
...
const getLedModelParams = (c: EngineComponent): LedModelParams => {
  ...
  const saturationCurrent = Math.max(1e-18, c.props.saturationCurrent ?? DEFAULT_LED_IS);
  const idealityFactor = Math.min(4, Math.max(1, c.props.idealityFactor ?? DEFAULT_LED_IDEALITY));
